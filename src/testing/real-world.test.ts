/**
 * Real-world Prisma test suite.
 *
 * Exercises every PostgreSQL-specific pattern that breaks when mocked:
 * - Enum columns and transitions
 * - Json/JsonB storage and querying
 * - Decimal precision across writes and reads
 * - String[] arrays with GIN index queries
 * - Bytes (binary data)
 * - Nested creates with relations
 * - findFirst/findMany with deep include
 * - Upsert on composite unique constraints
 * - Cascading deletes
 * - Nullable field semantics (null vs undefined — the nullish-coalescing trap)
 * - Batch operations
 * - updatedAt auto-generation
 * - Transactions
...
const createWorkspace = async (tenantId: string, slug = 'test-ws') => {
...
describe('Json and JsonB columns', () => {
  it('stores and retrieves Json objects', async () => {
    ...
    const updated = await prisma.tenant.update({
      where: { id: tenant.id },
      data: {
        config: { theme: 'dark', notifications: { email: true, slack: false } },
      },
    });
...
describe('Bytes (binary data)', () => {
  ...
  it('handles large binary data', async () => {
    const content = Buffer.alloc(64 * 1024, 0xab); // 64KB
    const blob = await prisma.blob.create({
