...
describe('PGliteBridge (class)', () => {
  it('throws when constructed with a not-yet-ready PGlite', () => {
    ...
    expect(() => new PGliteBridge({ pglite: notReady })).toThrow(
      /requires a ready PGlite instance/,
    );
  });

  it('throws when constructed with a closed PGlite', () => {
    const closed = createReadyMockPGlite();
    Object.assign(closed, { closed: true });
    expect(() => new PGliteBridge({ pglite: closed })).toThrow(/requires an open PGlite instance/);
  });
...
describe('PGliteBridge (class) — mocked pg.Pool', () => {
  type ClassModule = typeof import('./pglite-bridge-class.ts');
  ...
  const loadClassWithMocks = async ({
    ...
    return {
      PoolCtor,
      module: await import('./pglite-bridge-class.ts'),
      poolEnd,
      prismaPg,
    };
  };
