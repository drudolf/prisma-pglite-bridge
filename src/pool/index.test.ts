...
describe('PgBridgePool — rollback abandoned transaction on release', () => {
  ...
  // Robust teardown for a pool over a caller-owned `local` PGlite when a
  // client may have been abandoned mid-transaction. After `pool.end()`
  // resolves, the abandoned client's `_destroy` ROLLBACK can still be settling
  // inside PGlite's WASM `runExclusive`; calling `local.close()` immediately
  // closes the WASM instance mid-rollback and spins the event loop (a hang that
  // only surfaces in-process, e.g. under vitest). A direct query serializes
  // behind the in-flight rollback via runExclusive, so it is a deterministic
  // barrier rather than a timing race, and both bounded calls let a genuine
  // red-phase wedge fail the test instead of hanging. Once the fix lands the
  // barrier query is a no-op (nothing is in-flight), so this teardown is
  // correct under both current and fixed code.
  const endPoolAndClose = async (pool: PgBridgePool, local: PGlite): Promise<void> => {
    ...
    // direct query serializes behind it, so close() cannot race it (closing
    // PGlite mid-rollback kills the WASM instance and spins the event loop —
    // the known dead-WASM defect). A settle tick is a race, not a barrier.
    await local.query('SELECT 1').catch(() => {});
    if (!local.closed) await settledOrPending(local.close(), 10_000).catch(() => {});
  };
  ...
  it('unblocks a sibling with a clean view and recycles the client outside any transaction (E4)', async () => {
    ...
    try {
      ...
      // stays idle-in-tx; endPoolAndClose's settle tick drains the teardown
      // rollback before local.close() to avoid the WASM-mutex deadlock.
      await recycled?.query('ROLLBACK').catch(() => {});
      recycled?.release();
      await endPoolAndClose(pool, local);
    }
  });
  ...
  it('named statements promoted inside a transaction survive its ROLLBACK (probe-confirmed PG parity)', async () => {
    ...
    try {
      ...
      try {
        ...
        const inTx = await local.query<{ name: string }>(
          "SELECT name FROM pg_prepared_statements WHERE name LIKE 'ppb_%'",
        );
        ...
        const afterRollback = await local.query<{ name: string }>(
          "SELECT name FROM pg_prepared_statements WHERE name LIKE 'ppb_%'",
        );
